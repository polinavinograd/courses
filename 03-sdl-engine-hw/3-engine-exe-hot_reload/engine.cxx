...
game* reload_game(
  game* old, engine* eng, std::string lib, std::string temp_lib, void*& game_handle) {
  // std::cerr << "Start lib update!\n";
  if (old) {
  ...
  game_handle = SDL_LoadObject(temp_lib.c_str()); // exception
  if (!game_handle) {
  ...
  auto create_game =
    reinterpret_cast<game* (*)()>(SDL_LoadFunction(game_handle, "_ZN2my11create_gameEv"));
  if (!create_game) {
  ...
  new_game->init(eng);
  // std::cerr << typeid(*new_game).name() << std::endl;
  return new_game;
}
} // namespace my
...
int main(int argc, char* argv[]) {
  std::unique_ptr<my::engine> engine = my::engine_builder::create_engine();

  std::string libname = "./libgame_lib.so";
  std::string templib = "./templib.so";

  void* game_lib_handle = nullptr;
  my::game* current_game = reload_game(nullptr, engine.get(), libname, templib, game_lib_handle);
  ...
  while (!quit) {
    ...
    if (last_changed != time) {
      current_game = reload_game(current_game, engine.get(), libname, templib, game_lib_handle);
      time = last_changed;
      if (!current_game) {
    ...
    while (engine->check_input(event)) {
      current_game->on_event(event, quit);
    }
    current_game->update();
    current_game->render();
  }

  my::engine_builder::destroy_engine(engine);
  return EXIT_SUCCESS;
}